    ...
    <section className="py-12 px-6 md:px-16 overflow-hidden">
      <div className="flex flex-col xl:flex-row gap-12 xl:gap-24">
        ...
        <div className="w-full xl:w-[75%] flex flex-col gap-4 relative z-10">
          {isLoading ? (
            <div className="w-full h-24 bg-white/[0.02] border border-white/5 rounded-2xl animate-pulse"></div>
          ) : (
                ...
                <div className="flex items-start gap-4">
                  <div className="w-10 h-10 rounded-full bg-[#05070D] border border-white/10 flex items-center justify-center shrink-0 group-hover:border-[#143DED]/50 transition-colors">
                    {item.type === 'Achievement' ? (
                      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#143DED" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="8" r="6" /><path d="M15.477 12.89 17 22l-5-3-5 3 1.523-9.11" /></svg>
                    ) : (
                      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#143DED" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" /></svg>
                    )}
                  </div>
                  <div>
                    ...
                    <p className="font-body text-sm text-white/50">{item.issuer}</p>
                  </div>
                </div>
